...
from .OLD_skymap_tools import combine_images, get_skymap_lat_dist, get_skymap_fwhm, \
...
def plot_skymap_with_hists(ax_main, ax_histx, ax_histy, pb, tmp,
                           grid_x, grid_y,
                           int_x, int_y,
...
def plot_skymaps(tasks_to_plot, settings):
    ...
    for tmp in tasks_to_plot:
        ...
        for task in tmp:
            ...
            freq = plot_dict["freq"]
            time = plot_dict["time"]
            workingdir = plot_dict["workingdir"]
...
def plot_one_skymap_with_dists(task_to_plot, settings):
    ...
    if (len(settings["kn_skymap"].keys()) > 0):
        ...
        # i_zz_x /= i_zz_x.max()
        # i_zz_y /= i_zz_y.max()
        im = plot_skymap_with_hists(ax_main, ax_histx, ax_histy, pb1, tmp,
