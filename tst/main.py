...
def load_vinf_hist(rootpath="", fname="/hist_vel_inf.dat"):
    hist1 = np.loadtxt(rootpath + fname)
...
def load_corr_file2(rootpath,
                    fname="corr_vel_inf_theta.h5",
                    reinterpolate_theta=True,
                    new_theta_len=None,
                    beta_min=0., beta_max=0.
                    ):
    # load the corr file
    dfile = h5py.File(rootpath + fname, mode="r")
...
def compute_ek_corr(_vinf, _mass):
    ...
    return res
def prepare_kn_ej_id_2d(nlayers, outfpath):
    thetas, betas, masses = load_corr_file2(rootpath="",
                                            fname="corr_vel_inf_theta.h5",
                                            reinterpolate_theta=True,
...
def prepare_kn_ej_id_1d(nlayers, outfpath):
    betas, masses = load_vinf_hist()
