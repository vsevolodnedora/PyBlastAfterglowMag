...
def main():
    ...
    print(pba.main_pars)
    pba.modify_main_part_parfile(newpars=dict({"theta_obs":0.0}),newopts={})
    pba.modify_grb_part_parfile(newpars={},newopts={"fname_light_curve":"grb_lc_theta016.h5"})
    pba.run()
    # print(pba.main_pars)
    # print(pba.get_jet_lc_totalflux(freq=nu[0]))

    plt.loglog(pba.get_jet_dyn_arr("R",ilayer=0), pba.get_jet_dyn_arr("rho",ilayer=0))
    # plt.loglog(pba.get_jet_dyn_arr("R",ilayer=0), pba.get_jet_dyn_arr("Gamma",ilayer=0))
    plt.show()
    ...
    # ax.plot(t, ffs_afgpy, ls=':', color='gray', label='afterglopy')
    ax.plot(t/cgs.day, Fnu, ls=':', color='gray', label='afterglopy')

    # tts, fluxes = np.loadtxt(curdir+"./jelib_grb170817.txt",unpack=True)
    # ax.plot(tts, fluxes*1e26, ls='--', color='gray', label='Joelib')

    ax.plot(pba.get_jet_lc_times()/cgs.day, pba.get_jet_lc_totalflux(freq=nu[0]), ls='-', color="black", label="PBA")
    ...
    ax.tick_params(**{"axis": 'both', "which": 'both', "labelleft": True, 'labelright': False, "tick1On": True,
                      "tick2On": True, "labelsize": 11, "direction": 'in', "bottom": True, "top": True, "left": True,
                      "right": True})
    plt.show()
