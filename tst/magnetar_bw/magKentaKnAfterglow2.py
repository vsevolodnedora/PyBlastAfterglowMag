...
def run():
    ...
    # plot_init_profile(ctheta_[0,:], mom_[:,0], np.log10(r_),
    ...
    """ -----------| Prepare PWN Profile |---------- """
    # assume it is similar to ejecta profile just compressed into 1D and far more eneretic
    dfile_ej = h5py.File(workdir+f"corr_id_SFHo_13_14_150m_11_text{int(text)}.h5","r")

    dfile_mag = h5py.File(workdir+"magnetar_evol.h5","r")
    t0 = np.array(dfile_mag["time"])[0]
    e0 = t0 * (np.array(dfile_mag["lacc"])[0]+np.array(dfile_mag["ldip"])[0])
    print(f"t0={t0} s, e0={e0} ergs")

    # assuming M0 = E0 / (cgs.c ** 2 * Gamma0) -> Gamma0 = E0 / (cgs.c ** 2 * M0)
    # assuming M0 = sum(mass)
    m0 = mass_[0,:]#np.sum(mass_, axis=0)
    g0 = GamFromMom(mom_[0, :])
    ...
    dfile.create_dataset("theta",data=theta_[0,:])
    dfile.create_dataset("ctheta",data=ctheta_[0,:])
    dfile.create_dataset("mom",data=MomFromGam(g0))
    dfile.create_dataset("ek",data=etot)
    dfile.create_dataset("mass",data=m0)
    dfile.create_dataset("ye",data=ye_[0,:])
    dfile.create_dataset("s",data=s_[0,:])
    dfile.create_dataset("rho",data=np.zeros_like(theta_[0,:]))
    dfile.create_dataset("temp",data=np.zeros_like(theta_[0,:]))
    dfile.close()

    g0 = e0 / (cgs.c ** 2 * m0)

    # print(dfile_mag.keys())
    # print("Total ejected energy is (Lsd+Lacc)*dt = {}")
    # exit(1)
    #
    # pwn_r = np.array(dfile_ej["r"])[:,0]
    # pwn_ek = np.array(dfile_ej["ek"])[:,0] * 100
    #
    #
    #
    # prepare_grb_ej_id_1d({"Eiso_c":1.e52, "Gamma0c": 150., "M0c": -1.,"theta_c": np.pi/2., "theta_w": np.pi/2.,
    #                       "nlayers_pw": int(len(np.array(dfile_ej["r"])[:,0])),
    #                       "nlayers_a": int(len(np.array(dfile_ej["r"])[:,0])),
    #                       "struct":"tophat"},type="pw",outfpath="pwn_1d_id.h5")
    #
    # # load the file and add the R0 that corresponds to the R0 of the ejecta
    # dfile_ej = h5py.File(workdir+f"corr_id_SFHo_13_14_150m_11_text{int(text)}.h5","r")
    # dfile_pwn = h5py.File(os.getcwd()+'/'+"pwn_1d_id.h5","a")
    # print(dfile_ej.keys())
    # print(np.array(dfile_ej["r"])[:,0].shape)
    # print(np.array(dfile_pwn["r"]).shape)
    # exit(1)
