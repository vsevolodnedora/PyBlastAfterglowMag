...
from PyBlastAfterglowMag.parfile_tools import *
...
#              combine_images,combine_images_old,get_skymap_lat_dist,_plot_skymap_with_hists,get_skymap_fwhm)
#     except ImportError:
...
class TestBases():
    ...
    def run_pw(self, pars : dict, struct : dict, opts : dict, opts_grb : dict) -> PyBlastAfterglow:
        ...
        with h5py.File(self.workingdir+"gauss_grb_id.h5", "w") as dfile:
            for key, data in id_dict.items():
                dfile.create_dataset(name=key, data=data)
        # modify parfile
        PBA.parfile_tools.modify_parfile_par_opt(workingdir=self.workingdir, part="main",
                               newpars=pars,
                               newopts=opts,
                               parfile=parfilefname, newparfile="parfile.par", keep_old=True)
        PBA.parfile_tools.modify_parfile_par_opt(workingdir=self.workingdir, part="grb",
                               newpars=pars,
                               newopts=opts_grb,
        ...
        pba_pw = PBA.interface.PyBlastAfterglow(workingdir=self.workingdir,readparfileforpaths=True, parfile="parfile.par")
        # run
    ...
    def run_a(self, pars : dict, struct : dict, opts : dict, opts_grb : dict) -> PyBlastAfterglow:
        ...
        if not os.path.isfile(self.workingdir+"parfile.par"):
            raise FileNotFoundError(f"Parfile not found in {self.workingdir}"+"parfile.par")
        pba_a = PBA.interface.PyBlastAfterglow(workingdir=self.workingdir,readparfileforpaths=True, parfile="parfile.par")
...
def runset_for_skymap(tsk, default_parfile_fpath : str, workingdir : str, figdir : str):
    ...
    grb = TestCasesFS(default_parfile_fpath=default_parfile_fpath, workingdir=workingdir)
