//
// Created by vsevolod on 03/04/23.
//
        ...
        void computeMu(){
            for(size_t il = 0; il > p_cumShells->nlayers(); il++) {
                for (size_t ish = 0; ish < p_cumShells->nshells(); ish++) {
                    ...
                    if(data[BlastWaveBase::Q::imu][nr-1] < 1. ){
                        std::cout << data[BlastWaveBase::Q::itburst] << "\n";
                        std::cout << data[BlastWaveBase::Q::iR] << "\n";
                        (*p_log)(LOG_WARN,AT) << " mu[-1]=" << data[BlastWaveBase::Q::imu][nr-1] << " < 1 (expected >1) "
                                              << " tobs="<<t_obs
                                              << " tbutst[-1]="<< data[BlastWaveBase::Q::itburst][nr-1]
                                              << " R[nr-1]="<<data[BlastWaveBase::Q::iR][nr-1]
                                              << "\n";
                    }
                    if(data[BlastWaveBase::Q::imu][0] > 1. ){
                        (*p_log)(LOG_WARN,AT) << " mu[0]=" << data[BlastWaveBase::Q::imu][0] << " > 1 (expected <1) "
                                              << " tobs="<<t_obs
                                              << " tbutst[0]="<< data[BlastWaveBase::Q::itburst][0]
                                              << " R[0]="<<data[BlastWaveBase::Q::iR][0]
                                              << "\n";
                    }
                    //        std::cerr << AT
...
//        p_pars=std::make_unique<Pars>(p_cumShells, loglevel);
    ...
    double evalFluxDensA(double t_obs, double nu_obs, double atol) {
        double fluxdens = 0.;
        p_pars->parsPars(t_obs, nu_obs, p_pars->theta_c_l, p_pars->theta_c_h,
                 0., M_PI, obsAngle);
        p_pars->check_pars();
    ...
    void evalImagePW(Image & image, Image & im_pj, Image & im_cj, VecArray & m_data, size_t ish, size_t il, double ctheta0, double obs_time, double obs_freq){
        Array phi_grid = LatStruct::getCphiGridPW( il );
        computeImagePW(im_pj, im_cj, m_data, ish, il, ctheta0, obs_time, obs_freq );
    ...
    ///
    void evalImageFromPW(Image & image, VecArray & m_data, size_t ish, size_t il, double ctheta0,
                         double t_obs, double nu_obs,
                         double (*obs_angle)( const double &, const double &, const double & ),
...
//        auto & p_syn = p_pars->p_rs->getAnSynch();
        ...
        if ((m_data[BlastWaveBase::Q::iR][0] == 0.) && (m_data[BlastWaveBase::Q::iGamma][0] > 0)){
            (*p_log)(LOG_WARN,AT) << " [ishell=" << ish << " ilayer="<<il << "] "
                                  << " R[0]=0. Seems not evolved -> returning empty image." << "\n";
            return;
        }

        p_pars->parsPars(t_obs, nu_obs, 0., 0., 0., 0., obs_angle);
        p_pars->check_pars();
...
//        }
        ...
        Array cphis = LatStruct::getCphiGridPW(il);
...
//                    std::cout << ttobs << std::endl;
                ...
                if ((r <= 0.0) || !std::isfinite(r)) {
                    (*p_log)(LOG_ERR,AT) << " R <= 0. Extend R grid (increasing R0, R1). "
                                         << " Current R grid us ["
                                         << m_data[BlastWaveBase::Q::iR][0] << ", "
                                         << m_data[BlastWaveBase::Q::iR][p_pars->nr - 1] << "] "
                                         << "and tobs arr ["
...
//                double us = 4.0 * u * sqrt((u * u + 1) / (8. * u * u + 9.)); // from the blast wave velocity -> compute shock velocity
                ...
                double intensity = RadiationBase::computeIntensity(em_lab, dtau,
                                                                   SynchrotronAnalytic::METHOD_TAU::iSMOOTH);
                double flux_dens = (intensity * r * r * dr) * (1.0 + p_pars->z) / (2.0 * p_pars->d_l * p_pars->d_l);
                flux+=flux_dens;
                /// save the result in image
...
//                        std::cerr << AT << "\n";
                ...
                if ((r <= 0.0) || !std::isfinite(r)) {
                    (*p_log)(LOG_WARN,AT) << " R <= 0. Extend R grid (increasing R0, R1). "
                                          << " Current R grid us ["
                                          << m_data[BlastWaveBase::Q::iR][0] << ", "
                                          << m_data[BlastWaveBase::Q::iR][nr - 1] << "] "
                                          << "and tobs arr ["
                                          << ttobs[0] << ", " << ttobs[p_pars->nr - 1]
                                          << "] while the requried obs_time=" << p_pars->t_obs
                                          << "\n";
...
//                    exit(1);
    ...
    void computeImagePW(Image & im_pj, Image & im_cj, VecArray & m_data, size_t ish, size_t il, double ctheta0,
                        double obs_time, double obs_freq){
        auto & p_eats = p_pars; // removing EATS_pars for simplicity
        size_t cells_in_layer = LatStruct::CellsInLayer(il);
        /// compute image for primary jet and counter jet
        evalImageFromPW(im_pj, m_data, ish, il, ctheta0, obs_time, obs_freq, obsAngle, imageXXs, imageYYs);
        if (p_eats->counter_jet) // p_eats->counter_jet
            evalImageFromPW(im_cj, m_data, ish, il, ctheta0, obs_time, obs_freq, obsAngleCJ, imageXXsCJ, imageYYsCJ);
    }

    /// precompute electron distribution properties for each timestep
    void addComputeForwardShockMicrophysics(size_t it, VecArray & m_data){
//        auto & p_eats = p_pars; // removing EATS_pars for simplicity
//        auto & p_syn = p_pars->p_fs->getAnSynch();
        if ((m_data[BlastWaveBase::Q::ibeta][it] < p_pars->beta_min)){
            return;
        }
        if ((m_data[BlastWaveBase::Q::iCSCBM][it] >= m_data[BlastWaveBase::Q::ibeta][it])){
            m_data[BlastWaveBase::Q::iB][it] = 0.;
            return;
        }
        // no ISM case -> no electron acceleration
        if ((m_data[BlastWaveBase::Q::iU_p][it] <= 0) || (m_data[BlastWaveBase::Q::irho2][it] <= 0)){
...
//        }
        ...
        if (m_data[BlastWaveBase::Q::iGammaREL][it] > 0.) Gamma_ = m_data[BlastWaveBase::Q::iGammaREL][it];
        else Gamma_ = m_data[BlastWaveBase::Q::iGamma][it];
...
//            *tmp_time = m_data[Q::itburst][it];
        p_pars->p_syna->precompute(m_data[BlastWaveBase::Q::iU_p][it],
                          m_data[BlastWaveBase::Q::iGamma][it],
//                               m_data[Q::iGamma][it],
                          m_data[BlastWaveBase::Q::iGammaFsh][it],
                          m_data[BlastWaveBase::Q::itt][it], // TODO WHICH TIME IS HERE? tbirst? tcomov? observer time (TT)
//                               m_data[Q::itburst][it], // emission time (TT)
                          m_data[BlastWaveBase::Q::irho2][it] / CGS::mp);
        // adding
        m_data[BlastWaveBase::Q::iB][it]      = p_pars->p_syna->getPars()->B;
        m_data[BlastWaveBase::Q::igm][it]     = p_pars->p_syna->getPars()->gamma_min;
        m_data[BlastWaveBase::Q::igM][it]     = p_pars->p_syna->getPars()->gamma_max;
        m_data[BlastWaveBase::Q::igc][it]     = p_pars->p_syna->getPars()->gamma_c;
        m_data[BlastWaveBase::Q::iTheta][it]  = p_pars->p_syna->getPars()->Theta;
//        m_data[Q::ix][it]      = p_syn->getPars()->x; // IT is not evaluated/ It depends in freq
        m_data[BlastWaveBase::Q::iz_cool][it] = p_pars->p_syna->getPars()->z_cool;
        m_data[BlastWaveBase::Q::inprime][it] = p_pars->p_syna->getPars()->n_prime;
        m_data[BlastWaveBase::Q::iacc_frac][it] = p_pars->p_syna->getPars()->accel_frac;
//        if ( m_data[Q::iGamma][it] < 5 ){
//            exit(1);
//        }
        if ((!std::isfinite( m_data[BlastWaveBase::Q::igm][it] )) || (m_data[BlastWaveBase::Q::iB][it] < 0.)) {
            (*p_log)(LOG_ERR,AT) << " Wrong value at i=" << it << " tb=" << m_data[BlastWaveBase::Q::itburst][it]
                                 << " iR="     << m_data[BlastWaveBase::Q::iR][it] << "\n"
                                 << " iGamma=" << m_data[BlastWaveBase::Q::iGamma][it] << "\n"
                                 << " ibeta=" << m_data[BlastWaveBase::Q::ibeta][it] << "\n"
                                 << " iM2="    << m_data[BlastWaveBase::Q::iM2][it] << "\n"
                                 << " iEint2=" << m_data[BlastWaveBase::Q::iEint2][it] << "\n"
                                 << " iU_p="   << m_data[BlastWaveBase::Q::iU_p][it] << "\n"
                                 << " irho="   << m_data[BlastWaveBase::Q::irho][it] << "\n"
                                 << " irho2="  << m_data[BlastWaveBase::Q::irho2][it] << "\n"
                                 << " iB="     << m_data[BlastWaveBase::Q::iB][it] << "\n"
                                 << " igm="    << m_data[BlastWaveBase::Q::igm][it] << "\n"
                                 << " igM="    << m_data[BlastWaveBase::Q::igM][it] << "\n"
                                 << " igc="    << m_data[BlastWaveBase::Q::igc][it] << "\n"
                                 << " iTheta=" << m_data[BlastWaveBase::Q::iTheta][it] << "\n"
                                 //                      << " ix="     << m_data[Q::ix][it] << "\n"
                                 << " iz_cool="<< m_data[BlastWaveBase::Q::iz_cool][it] << "\n"
                                 << " inprime="<< m_data[BlastWaveBase::Q::inprime][it]
                                 << "\n";
            exit(1);
        }
    }
    void computeForwardShockElectronAnalyticVars(VecArray & m_data){
        for (size_t it = 0; it < p_pars->nr; ++it){
            addComputeForwardShockMicrophysics(it, m_data);
        }
...
//            exit(1);
    ...
    void computeForwardShockComovingEmissivityAndAbsorption(size_t it, VecArray & m_data){
//        auto & p_eats = p_pars; // removing EATS_pars for simplicity
//        auto & p_syn = p_pars->p_fs->getAnSynch();
...
//                              << p_syn->getFreqArr().size() << " grid";
        ...
        else beta_ = m_data[BlastWaveBase::Q::ibeta][it];

        /// if BW is not evolved to this 'it' or velocity is smaller than minimum
        if ((m_data[BlastWaveBase::Q::iR][it] < 1)||(beta_ < p_pars->beta_min))
            return;

        if (m_data[BlastWaveBase::Q::igm][it] == 0){
        ...
        for (size_t ifreq = 0; ifreq < p_pars->m_freq_arr.size(); ++ifreq){
            ...
            p_pars->p_syna->compute(m_data[BlastWaveBase::Q::irho2][it] / CGS::mp,//m_data[Q::iM2][it] / CGS::mp,
                           m_data[BlastWaveBase::Q::iM2][it] / CGS::mp,//m_data[Q::iM2][it] / CGS::mp,
                           m_data[BlastWaveBase::Q::iacc_frac][it],
                           m_data[BlastWaveBase::Q::iB][it],
                           m_data[BlastWaveBase::Q::igm][it],
                           m_data[BlastWaveBase::Q::igM][it],
                           m_data[BlastWaveBase::Q::igc][it],
                           m_data[BlastWaveBase::Q::iTheta][it],
                           m_data[BlastWaveBase::Q::iz_cool][it],
...
//            p_syn->addIntensity(thick_tau, 0., 1.);
            p_pars->m_synch_em[ifreq + p_pars->nfreq * it] = p_pars->p_syna->get_em();
            p_pars->m_synch_abs[ifreq + p_pars->nfreq * it] = p_pars->p_syna->get_abs();
        }
    }
    void computeForwardShockSynchrotronAnalyticSpectrum(VecArray & m_data){
        auto & p_eats = p_pars; // removing EATS_pars for simplicity
        if (p_eats->m_method_rad==METHODS_RAD::icomovspec) {
            (*p_log)(LOG_INFO,AT) << " computing analytic comoving spectrum\n";
            for (size_t it = 0; it < p_pars->nr; ++it) {
                computeForwardShockComovingEmissivityAndAbsorption(it, m_data);
            }
        }
    }
    ...
    auto evalForwardShockComovingSynchrotron(VecArray & m_data, Vector & freq_arr, size_t every_it ){
//        auto & p_eats = p_pars; // removing EATS_pars for simplicity
//        auto & p_syn = p_pars->p_fs->getAnSynch();
        Vector t_arr{};
        if (every_it == 0){
...
//            out_abs.resize(t_arr.size());
        ...
        data ( p_pars->p_syna->m_names_.size() );
        for (auto & arr : data) {
        ...
        for (size_t it = 0; it < p_pars->p_cumShells->getTbGrid().size(); it = it + every_it){
            ...
            if ((m_data[BlastWaveBase::Q::iR][it] < 1)||(beta_ < p_pars->p_syna->getPars()->beta_min))
                continue;
            if (m_data[BlastWaveBase::Q::igm][it] == 0){
                (*p_log)(LOG_ERR,AT) << " in evolved blast wave, found gm = 0" << "\n";
                exit(1);
            ...
            for (size_t ifreq = 0; ifreq < freq_arr.size(); ++ifreq){
                /// compute all types of emissivities and absoprtions
                p_pars->p_syna->compute(m_data[BlastWaveBase::Q::irho2][it]/CGS::mp,//m_data[Q::iM2][it] / CGS::mp,
                               m_data[BlastWaveBase::Q::iM2][it] / CGS::mp,
                               m_data[BlastWaveBase::Q::iacc_frac][it],
                               m_data[BlastWaveBase::Q::iB][it],
                               m_data[BlastWaveBase::Q::igm][it],
                               m_data[BlastWaveBase::Q::igM][it],
...
//                                       m_data[Q::irho2][it] / CGS::mp,
                ...
                /// add evaluated data to the storage
                double thick_tau = EQS::shock_delta(m_data[BlastWaveBase::Q::iRsh][it],m_data[BlastWaveBase::Q::iGammaFsh][it]);
//                p_syn->addIntensity(m_data[Q::ithickness][it], 0., 1.);
                p_pars->p_syna->addIntensity(thick_tau, 0., 1.);
                double val;
                for (size_t i_type = 0; i_type < p_pars->p_syna->m_names_.size(); ++i_type) {
                    val = p_pars->p_syna->getData()[i_type];
...
//            data.emplace_back(evalForwardShockComovingSynchrotron(freq, itype, every_it));
    ...
    void evalForwardShockLightCurve(Image & image, Image & im_pj, Image & im_cj, VecArray & m_data, size_t ish, size_t il, double ctheta0,
                                    Vector & light_curve, Vector & times, Vector & freqs ){
//        Vector light_curve (times.size(), 0.0);
        if ((m_data[BlastWaveBase::Q::iR][0] == 0.0) && (m_data[BlastWaveBase::Q::iR][p_pars->nr-1] == 0.0)){
            (*p_log)(LOG_WARN, AT)
                    << " blast wave not evolved, flux=0 [ishell="<<ish<<", ilayer="<<il<<"]\n";
...
//                    auto image = model->evalImagePW(obs_times[it], obs_freqs[it]);
                computeImagePW(im_pj, im_cj, m_data, ish, il, ctheta0, times[it], freqs[it] );
                image.m_f_tot = (im_pj.m_f_tot + im_cj.m_f_tot);
//                evalImagePW(image, im_pj, im_cj, times[it], freqs[it]);
                light_curve[it] += image.m_f_tot;
            }
            else{
                double atol = light_curve[it] * rtol / (double)il;
