...
#ifndef SRC_QUADRATURES_H
#define SRC_QUADRATURES_H
 ...
 *
 * @param x values of the coordinates
 ...
 */
template <typename T>
double Simpson38( double x0, double x1, int n_elements, T & func ){

    double interval_size = (x1 - x0) / n_elements;
    double sum = func(x0) + func(x1);
    for (int i = 1 ; i < n_elements ; i++) {
        if (i % 3 == 0)
            sum = sum + 2 * func(x0 + i * interval_size);
        else
 ...
 * by Geoffrey Ryan
 *
 * This is a set of adaptive and fixed-grid 1D quadratures
 *
 */

enum METHODS_QUADRATURES {INT_TRAP_FIXED, INT_TRAP_ADAPT, INT_SIMP_FIXED, INT_SIMP_ADAPT,
...
/* QUADRATURES FROM afterglopy :: quadratures */
               ...
               double *I, double *err)
{
    ...
    double err2 = Iegk * std::pow(200 * fabs(Igk - Ig) / Iegk, 1.5);
    if(err2 > Iegk)
        err2 = Iegk;
