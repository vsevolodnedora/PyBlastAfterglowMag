...
def plot_lcs(tasks, model, data:Data, figfpath, ylim:tuple or None = None):
    times = data.times
    freqs = data.freqs
    ...
    fig, axes = plt.subplots(2, len(tasks), figsize=(12,3.5),
                             gridspec_kw={'height_ratios': [3, 1]},
                             layout='constrained',sharey='row',sharex='col')
    ...
    for i, task in enumerate(tasks):
        ...
        for freq in freqs:
            ...
            l11, = axes[0, i].plot(times/86400, 10**lc*1e3,ls='-', color='gray', alpha=0.5, label=f"NR") # for second legend
            l21, = axes[0, i].plot(times/86400, 10**lc*1e3,ls='-', color=cmap(norm(freq)), alpha=0.5, label=f"{freq/1e9:.1f} GHz")
            l2s.append(l21)
            ...
            l12, = axes[0, i].plot(times/86400, 10**lc_nn*1e3,ls='--', color='gray', alpha=0.5, label=f"surrogate") # for second legend
            l22, = axes[0, i].plot(times/86400, 10**lc_nn*1e3,ls='--', color=cmap(norm(freq)), alpha=0.5)
        ...
        axes[1, i].set_xlabel(r"$t_{\rm obs}$ [day]",fontsize=12)
        if ylim : axes[0, i].set_ylim(*ylim)
        title = []
        if 'title' in task.keys(): axes[0,i].set_title(task['title'],fontsize=12)
    for axs in axes:
        for ax in axs:
    ...
    first_legend = axes[0,-1].legend(handles=l2s,
                                    **dict(fancybox=False,loc= 'upper right',columnspacing=0.4,
                                           #"bbox_to_anchor": (0.5, 1.2),  # loc=(0.0, 0.6),  # (1.0, 0.3), # <-> |
                                           shadow=False, ncol= 2, fontsize= 12,framealpha=0., borderaxespad= 0., frameon=False))

    axes[0,-1].add_artist(first_legend)
    axes[0,0].legend(handles=[l11,l12],
...
def main_run(fpath_to_collated:str=os.getcwd()+'/'+"collated.paraquet",
    ...
    df = pd.read_parquet(fpath_to_collated, index_col=0)
    ...
    if sample is None:
        ...
        plot_lcs(tasks, model, data, optuna_study_dir+"lcs")
...
if __name__ == '__main__':
    tasks = [
        ...
        {"name":"SFHoTim276_13_14_0025_150mstg_B0_HLLC","text":[None]},
    ...
    for task in tasks:
        ...
        for text in texts:
            for method in ["xgboost"]: # ", rf"
                ...
                main_run(
                    fpath_to_collated=root_path+"collated.paraquet",
