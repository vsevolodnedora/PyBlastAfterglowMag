...
grid_runs_dir = os.getcwd()+'/'+"grid_workingdirs/"
grid_parameters = dict(
    n_ism=    np.array([ 0.025,0.015 ]),
    theta_obs=np.array([ 20.,25. ]) * np.pi / 180.0,
    Eiso_c=   np.array([ 1e54,2e54 ]),
    Gamma0c=  np.array([ 300. ]),
    theta_c=  np.array([ 3.,4.]) * np.pi / 180.,
    theta_w=  np.array([ 22.,28. ]) * np.pi / 180.,
    p_fs=     np.array([ 2.2, 2.3 ]),
    eps_e_fs= np.array([ 0.01,0.001 ]),
    eps_b_fs= np.array([ 1e-3,1e-4 ]),
)
# ------------ RUN ANALYSIS
    ...
    def __call__(self, idx):
        ...
        if is_exists and not self.overwrite:
            ...
            compare_lcs_and_skymap_props(
                pba=pba,
                plot = dict(xlim=(4, -4), ylim=(-2, 2), show=False, figname="summary",
                outpath=working_dir,show_subluminal=False)
            )
...
def run_simulations():
    ...
    pars = copy.deepcopy(grid_parameters)
    ...
    for combination in all_combinations:
        ...
        pars_cobinations[-1]["name"] = "".join([par.replace("_","")+PBA.get_str_val(par,val)+'_'
                                                for par,val in zip(pars.keys(),combination)])
    print(f"Total runs to be performed {len(pars_cobinations)}")

    dispatcher = DispatcherRunPyBlastAfterglow(default_pars=P, list_pars=pars_cobinations, overwrite=False)

    # perform simulations in parallel
    start_time = timer.perf_counter()
    with Pool(ncpus) as pool:
        pool.map(dispatcher, range(len(pars_cobinations)))
    finish_time = timer.perf_counter()
