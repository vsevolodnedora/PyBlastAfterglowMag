...
def _normalize_spec(spec:np.ndarray, xs:np.ndarray, ys:np.ndarray, norm_method:None or str, mask_negatives=True):
    ...
    if norm_method == "/integ *y^2":
        ...
        integ = integrate.simps(y=spec, x=ys, axis=1)
        spec = spec / integ[:,np.newaxis]
        spec = spec * np.power(ys, 1)[np.newaxis,:]
...
class PlotSpectra:
    ...
    def _plot_rad_spectrum(ax, fig, xs:np.ndarray, ys:np.ndarray, spec:np.ndarray,
        ...
        if task_i["plot_num"]:
            ...
            ax.plot(xs,val,color='black', linewidth=1, linestyle=":",
...
def tasks_fs(do_run:bool, plot:bool, struct:dict, P:dict) -> PBA.PyBlastAfterglow:
