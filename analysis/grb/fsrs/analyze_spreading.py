...
if __name__ == '__main__':
    ...
    P=dict(main=dict(n_ism = 100., tb0=1e4, ntb=1000,rtol=1e-6,
                     lc_freqs = "array 1e9 1e18"),
           grb=dict(save_dynamics='yes',do_rs='yes',bw_type='fsrs',do_ele = "no",do_lc = "no",
                    do_rs_radiation="no",
                    # method_spread='None'
                    # exponential_rho4='no'
                    )
    ...
    plot_dyn(ax=ax,ej=pba_default.GRB,v_n_x="R",v_n_y="theta",layers=(0,6,12,19),
             plot_layer=dict(cmap="jet",ls='-'))
    plot_dyn(ax=ax,ej=pba_gp12.GRB,v_n_x="R",v_n_y="theta",layers=(0,6,12,19),
             plot_layer=dict(cmap="jet",ls='--'))
    plot_dyn(ax=ax,ej=pba_hu00.GRB,v_n_x="R",v_n_y="theta",layers=(0,6,12,19),
             plot_layer=dict(cmap="jet",ls=':',lw=1.5))
    plot_dyn(ax=ax,ej=pba_afgpy.GRB,v_n_x="R",v_n_y="theta",layers=(0,6,12,19),
             plot_layer=dict(cmap="jet",ls='-.',lw=2.))
    ax.set_xscale("log")
    plt.show()
