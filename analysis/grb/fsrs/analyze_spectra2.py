...
def plot_spectra_evolution(ej:PBA.Ejecta, fs_or_rs:str, tasks:dict,title:str or None,
    ...
    for v_n, task in tasks.items():
        if v_n in ["n_ele","gam_dot_syn","gam_dot_adi","gam_dot_ssc","yg"]:
            ...
            pl._plot_ele_spectrum(ax=axes[i_plot], fig=fig, times=times, ys=ys, spec=spec, ej=ej, fs_or_rs=fs_or_rs, task_i=task)
        else:
            ys, times, spec = pl._get_spectrum(ej=ej,v_n=v_n,fs_or_rs=fs_or_rs,norm_method=task["norm_method"])
            pl._plot_rad_spectrum(ax=axes[i_plot], times=times, ys=ys, spec=spec, fig=fig, ej=ej, fs_or_rs=fs_or_rs, v_n=v_n, task_i=task)
        i_plot += 1
...
def plot_spectra_evolution_ratio(ej1:PBA.Ejecta, ej2:PBA.Ejecta, v_n:str, fs_or_rs:str, task:dict):
    ...
    spec = spec1 / spec2
    if v_n == "n_ele":
        pl._plot_ele_spectrum(ax=ax, fig=fig, times=times, ys=ys, spec=spec, ej=ej1, fs_or_rs=fs_or_rs, task_i=task[v_n])
        # if task_i["plot_gm"]:
        ...
        pl._plot_rad_spectrum(ax=ax, fig=fig, times=times, ys=ys, spec=spec, ej=ej1, fs_or_rs=fs_or_rs, v_n=v_n, task_i=task[v_n])
...
def plot_emission_region_prop(ej:PBA.Ejecta,fs_or_rs:str,xlim:tuple,task:dict,ishell=0,ilayer=0):
    ...
    tot_ele = np.trapz(spec*gams[:,np.newaxis]/vol[np.newaxis,:]*PBA.utils.cgs.me*PBA.utils.cgs.c**2,x=gams,axis=0)

    gams, times, spec = sp._get_spectrum(ej=ej,v_n="syn_f",fs_or_rs=fs_or_rs,
                                    ishell=ishell,ilayer=ilayer,sum_shells_layers=False,norm_method=None)
    tot_syn = np.trapz(PBA.utils.cgs.h*spec*gams[:,np.newaxis],x=gams,axis=0) # * PBA.utils.cgs.h*spec*ys[np.newaxis,:]

    sp = PlotSpectra()
    gams, times, spec = sp._get_spectrum(ej=ej,v_n="ssc_f",fs_or_rs=fs_or_rs,
                                    ishell=ishell,ilayer=ilayer,sum_shells_layers=False,norm_method=None)
    tot_ssc = np.trapz(PBA.utils.cgs.h*spec*gams[:,np.newaxis],x=gams,axis=0) # PBA.utils.cgs.h*spec*ys[np.newaxis,:]
...
def plot_cooling_terms(ej:PBA.Ejecta,fs_or_rs:str,xlim:tuple,task:dict,ishell=0,ilayer=0):
    ...
    val1 = np.array((gam_dot_ssc>gam_dot_syn) * (gam_dot_ssc>gam_dot_adi), dtype=np.float64)
    val1[val1 < 1] = np.nan
    cs = ax.contourf(times, gams, val1  , hatches=['++'],  alpha=0.0, edgecolor = "r",facecolor="blue", color = "green") #,zorder=0
...
def tasks_fs(do_run:bool, plot:bool, struct:dict, P:dict, name:str="dyn_fs__rad_fs__num__ssa__ssc") -> PBA.PyBlastAfterglow:
...
if __name__ == '__main__':
    ...
    P = dict(
        ...
        grb=dict(save_dynamics='yes',do_spec='yes',save_spec='yes',do_lc='yes',
                 ...
                 # num_ele_use_adi_loss_fs='no',
                 # num_ele_use_adi_loss_rs='no',
                 gam1_fs=1.,gam2_fs=1e8,ngam_fs=801,
                 gam1_rs=1.,gam2_rs=1e8,ngam_rs=801,
                 freq1_fs=1e6,freq2_fs=1e32,nfreq_fs=400,
                 freq1_rs=1e6,freq2_rs=1e32,nfreq_rs=400)
    )
